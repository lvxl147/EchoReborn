...
static NSString *LGGlassPreferencesPath(void) {
    ...
    dispatch_once(&once, ^{
        path = jbroot(@"/var/mobile/Library/Preferences/com.strive.echoreborn.preferences.plist");
    });
    return path;
}
...
static BOOL LGNeedsGaussianIdentityFallback(void) {
    return access("/var/mobile/Library/Accessibility/echoreborn-liquidass-gaussian-identity-state.bin",
                  F_OK) == 0;
}
...
static CFStringRef const kLGMotionPrefsReloadNotification = CFSTR("com.strive.echoreborn/ReloadPrefs");
...
- (instancetype)initWithFrame:(CGRect)frame groupName:(NSString *)groupName filterType:(NSString *)filterType {
    ...
    if (groupName.length) {
        ...
        _lgGroupName = [NSString stringWithFormat:@"echoreborn.liquidglass.p%d.%08x.g%u",
                                                  getpid(), salt, _lgId];
    }
...
- (void)applyFilters {
    ...
    @try {
        ...
        if (!_backdropConfigured) {
            ...
            [layer setValue:@"echoreborn.liquidglass" forKey:@"groupNamespace"];
